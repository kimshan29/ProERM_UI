mainApp.controller("krCtrl", function ($scope, $routeParams, $timeout, $cookies, $injector, $location, HttpRequest, Helper, Constant, Model) {
    ...
    $scope.renderKrHeader = function()
    {
	    ...
	    HttpRequest.get(apiUrl).success(function(response) {
		    ...
		    apiUrl = "/api/KRListRiskOwner/" + $scope.krHeader.data.entitas.id;
		    HttpRequest.get(apiUrl).success(function(response) {
    ...
    $scope.id.renderMaster = function(indexIdentifikasi, indexDetailKatRisk)
    {
		...
		apiUrl = "/api/krListSubKategoriRisiko/" + idKatRisiko;
		...
		apiUrl = "/api/krListRisiko/" + idSubKatRisiko;
		...
		apiUrl = "/api/krListKejadianRisiko/" + idRisiko;
    ...
    $scope.id.listPenyebab = function (keyword) {
        var apiUrl = "/api/krListPenyebab/" + currentUser.idUnit + "?keyword=" + keyword;
    ...
    $scope.ae.renderListPeringkatDampakIR = function(indexAE, indexPenyebab, isAppended)
    {
    	...
    	var apiUrl = "/api/krListPeringkatDampak/" + idAreaDampak + "?efektifitas=" + nilai;
    ...
    $scope.ae.renderListPeringkatKemungkinanIR = function(indexAE, indexPenyebab, isAppended)
    {
    	...
    	try{
    		if ($scope.ae.data[indexAE].listPenyebab[indexPenyebab].existingControl.length > 0)
    			nilai = Helper.ifNullOrEmpty(Math.max.apply(Math, $scope.ae.data[indexAE].listPenyebab[indexPenyebab].existingControl.map(function(item){return item.nilai;})), 0);
    	}
    	catch(err){
    	}
    ...
    $scope.rm.renderListPeringkatDampak = function(indexRM, indexPenyebab, indexRencanaMitigasi, isAppended)
    {
        ...
        try {
            ...
            if (existingControlAE > 0)
                nilai = Helper.ifNullOrEmpty(Math.max.apply(Math, existingControlAE.map(function (item) { return item.nilai; })), 0);
        }
        catch (err) {
        }
    ...
    $scope.rm.listPeringkatDampak = function(indexRM, indexPenyebab)
    {
    	...
    	var apiUrl = "/api/krListPeringkatDampak/" + idAreaDampak;
	...
	$scope.id.kategoriRisikoChange = function(indexIdentifikasi, indexDetailKatRisk)
	{
		...
		apiUrl = "/api/krListSubKategoriRisiko/" + idKatRisk;
	...
	$scope.id.subKategoriRisikoChange = function(indexIdentifikasi, indexDetailKatRisk)
	{
		...
		apiUrl = "/api/krListRisiko/" + idSubKatRisk;
	...
	$scope.id.risikoChange = function(indexIdentifikasi, indexDetailKatRisk)
	{
		...
		HttpRequest.get(apiUrl).success(function(response){
			...
			$scope.id.master.kejadian[indexIdentifikasi][indexDetailKatRisk] = response[0];
			console.log(JSON.stringify($scope.id.master.kejadian[indexIdentifikasi][indexDetailKatRisk]));
			//console.log("indexDetailKatRisk : ",JSON.stringify(indexDetailKatRisk));
			var idKejadian = $scope.id.master.kejadian[indexIdentifikasi][indexDetailKatRisk].id;
			//$scope.id.data[indexIdentifikasi].detailRisiko[indexDetailKatRisk].kejadian == undefined ? Constant.emptyGuid : $scope.id.data[indexIdentifikasi].detailRisiko[indexDetailKatRisk].kejadian.id;
			console.log(JSON.stringify(idKejadian));
			$scope.id.kejadianChange(indexIdentifikasi, indexDetailKatRisk, idKejadian);
		})
        .error(function (response, code) {
            var data = {
                title: "List Kejadian Risiko",
	...
	$scope.id.kejadianChange = function(indexIdentifikasi, indexDetailKatRisk)
	{
		...
		$scope.id.data[indexIdentifikasi].detailRisiko[indexDetailKatRisk].kejadian.areaDampak = Helper.isNullOrEmpty(selectedKejadian) ? {id:"", name:""} : selectedKejadian.areaDampak;
	}
	...
	$scope.ae.toggleExistingControlAndCR = function(indexAE, indexPenyebab)
	{
		...
		if (trIR == "Rendah" || trIR == "Moderat") 
		{
			...
			$scope.ae.data[indexAE].listPenyebab[indexPenyebab].tipePeringkatKemungkinanCR = {id:Constant.emptyGuid, name:""};
